...
function reorderFrenchColorPhrase(value = '') {
  ...
  out = out.replace(/^(.*?)\s+de couleur\s+([a-zA-ZÀ-ÿ-]+)(.*)$/i, (_match, subject, color, suffix) => {
...
function stripImageGenerationCommandPrefix(value = '') {
  return String(value || '')
    .replace(/^(?:(?:tu peux|peux[- ]?tu|tu pourrais|pourrais[- ]?tu|je veux|je voudrais|j aimerais|j'aimerais)\s+)+(?:me\s+)?/i, '')
    .replace(/^(?:peux[- ]?tu\s+)?(?:s(?:tp|il te plait|’il te plait|il te plaît)\s+)?/i, '')
    .replace(/^(?:genere|g[eé]n[eéè]r(?:e|er|é|ée)|generate(?:d)?|cree|cr[eé]e(?:r|é|ée)?|dessine(?:r|é|ée)?|fabrique(?:r|é|ée)?|produis|produire|prepare|pr[eé]par(?:e|er|é|ée)|montre|affiche)\s+(?:moi\s+)?(?:une?\s+)?(?:image|illustration|dessin|photo|visuel|portrait)\s+(?:de|du|de la|de l['’]?|d['’]|\bd\s+)\s*/i, '')
...
function trimSubjectPhrase(value = '') {
  return normalizeWhitespace(String(value || '')
    ...
    .replace(/\b(?:dans|sur|avec|au milieu de|au bord de|sortant|sorti|sortie|coming out of|in|on|with)\b.*$/i, '')
...
function buildAnimalColorBinding(details = {}) {
  ...
  const normalizedBareSubject = normalizeIntentText(bareSubject);
  const sheepLikeSubject = /\b(sheep)\b/.test(normalizedBareSubject);

  return {
    promptLead: sheepLikeSubject
      ? `close-up portrait of one ${bareSubject} with ${stabilizedColor} ${surface}`
      : `one ${bareSubject} with ${stabilizedColor} ${surface}`,
    structuralAtoms: [
      ...
      ...(sheepLikeSubject
        ? ['single sheep portrait', 'isolated sheep', 'no flock', 'no herd', 'no background sheep']
        : []),
    ],
...
function compileSingleSubjectConstraints(rawPrompt = '', translatedPrompt = '') {
  ...
  const translatedSubject = normalizeWhitespace(translatedPrompt || translateImagePromptToEnglish(subjectPhrase));
