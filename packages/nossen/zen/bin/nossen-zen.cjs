...
function usage() {
  return [
    ...
    '  nossen-zen inspect --in archive.zen [--json]',
    '  nossen-zen verify --in archive.zen --key-env ZEN_KEY [--json]',
    '',
    'Dev fixtures only:',
...
async function main() {
  ...
  if (command === 'encode') {
    ...
    const written = await encodeZenFileAsync(input, outputPath, {
      ...limits,
      key: resolveKey(args),
  ...
  if (command === 'decode') {
    ...
    const decoded = await decodeZenFileAsync(input, outputPath, { ...limits, key: resolveKey(args) });
  ...
  if (command === 'verify') {
    const verification = await verifyZenFileAsync(input, { ...limits, key: resolveKey(args) });
