...
## Canonical MCP Roster

The current multi-agent MCP roster lives here:

- Human-readable: `docs/ops/MCP_ROSTER.md`
- Machine-readable: `docs/ops/mcp-roster.json`
- Repair runbook: `docs/ops/OPERATION_BB_READINESS_2026-05-15.md`

Use those files as the source of truth for agent endpoint, auth mode, allowed tool bundles, blocked tools, and Operation BB readiness.
...
On every new connection, agents should behave like a teammate arriving at a
shared desk:

1. Send an MCP heartbeat.
2. Check open discussions/inbox.
...
- `a11/docs/A11_SEMANTIC_RESONANCE_ENGINE.md`
- `a11/runtime/knowledge-graph/semantic-resonance-seeds.json`
- `tasks/semantic-resonance.md`

Rule: a reference is not only a fact. It can carry sensory memory, culture,
emotion, wordplay, archetype and interaction history. Use that layer to
understand the user better, while keeping answers clear and grounded.
