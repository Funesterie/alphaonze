  ...
  global.fetch = async (url, options = {}) => {
    ...
    if (value === 'http://127.0.0.1:5002/api/tts') {
      ...
      return {
        ...
        status: 200,
        async text() {
          return JSON.stringify({ audio_url: '/out/basic-a11-source.wav', via: 'http' });
        },
      };
    }
    if (value === 'http://127.0.0.1:5002/out/basic-a11-source.wav' || value === 'http://127.0.0.1:5002/out/basic-a11-source.wav?consume=1') {
      return {
        ok: true,
        status: 200,
...
test('tts speak route accepts explicit A11 voix de lait style', async () => {
  ...
  const previousFetch = global.fetch;
  const wav = createPcm16Wav({ frequency: 150 });
  const conversionForms = [];
  ...
  global.fetch = async (url, options = {}) => {
      ...
      return {
        ...
        headers: {
          get(name) {
            const header = String(name || '').toLowerCase();
            if (header === 'content-type') return 'audio/wav';
            if (header === 'x-a11-voice-style') return 'a11-voix-de-lait';
            if (header === 'x-a11-voice-engine') return 'xtts-rvc';
            return '';
          },
        },
    ...
    if (value.includes('/tts/bytes')) {
      throw new Error('cartesia_should_not_be_called_for_a11_voix_de_lait');
    }
    if (value.startsWith('https://api.elevenlabs.test/v1/')) {
      throw new Error('elevenlabs_should_not_be_called_for_a11_voix_de_lait');
    }
    return previousFetch(url, options);
  };
  ...
  try {
    await withServer(
      ...
      async (baseUrl) => {
        ...
        assert.equal(conversionForms.length, 1);
        assert.equal(conversionForms[0].get('persona'), 'a11');
        assert.equal(conversionForms[0].get('voiceStyle'), 'a11-voix-de-lait');
        assert.equal(result.json.voiceConversion.voiceStyle, 'a11-voix-de-lait');
      }
    );
  } finally {
    global.fetch = previousFetch;
...
test('tts async official voice jobs can be claimed and completed by the local GPU worker', async () => {
  const previousEnv = {
    ...
    CARTESIA_API_KEY: process.env.CARTESIA_API_KEY,
    CARTESIA_TOKEN: process.env.CARTESIA_TOKEN,
    A11_CARTESIA_API_KEY_FILE: process.env.A11_CARTESIA_API_KEY_FILE,
    CARTESIA_API_KEY_FILE: process.env.CARTESIA_API_KEY_FILE,
    A11_AZURE_SPEECH_KEY: process.env.A11_AZURE_SPEECH_KEY,
  ...
  delete process.env.CARTESIA_API_KEY;
  delete process.env.CARTESIA_TOKEN;
  delete process.env.A11_CARTESIA_API_KEY_FILE;
  delete process.env.CARTESIA_API_KEY_FILE;
  delete process.env.A11_AZURE_SPEECH_KEY;
...
test('vivy jobs route exposes a Bat/Rome async official TTS job with web audio output', async () => {
  ...
  global.fetch = async (url, options = {}) => {
    ...
    if (value === 'https://api.cartesia.test/tts/bytes') {
      throw new Error('cartesia_should_not_run_for_vivy_official_job');
    }
    ...
    if (value === 'http://voice-bridge.test/api/voice/convert') {
      ...
      return {
        ...
        async arrayBuffer() {
          return createPcm16Wav({ frequency: 220 });
        },
      };
    }
...
test('vivy jobs route keeps stale private voice state away from the local GPU worker', async () => {
  ...
  global.fetch = async (url, options = {}) => {
    const value = String(url);
    if (value === 'https://api.cartesia.test/tts/bytes') {
      throw new Error('cartesia_should_not_run_for_stale_vivy_private_state');
    }
    if (value === 'http://voice-bridge.test/api/voice/synthesize') {
      return {
        ok: false,
    ...
    if (value === 'http://voice-bridge.test/api/voice/convert') {
      ...
      return {
        ...
        async arrayBuffer() {
          return createPcm16Wav({ frequency: 210 });
        },
      };
    }
    if (value === 'http://a11-voice:5002/api/tts') {
...
test('tts speak route can use Cartesia when selected explicitly by an authorized voice request', async () => {
  ...
  process.env.A11_LOCAL_XTTS_RVC_AUTODETECT = '0';
  process.env.A11_CARTESIA_API_KEY = 'test-cartesia-key';
  process.env.A11_CARTESIA_TTS_ENABLED = 'true';
  process.env.A11_CARTESIA_BASE_URL = 'https://api.cartesia.test';
  process.env.ENABLE_PIPER_HTTP = 'true';
  process.env.A11_VOICE_MODULE_URL = 'http://a11-voice:5002';
  delete process.env.A11_XTTS_RVC_URL;
  ...
  try {
    await withServer(
      ...
      async (baseUrl) => {
        ...
        assert.match(result.json.voiceReference.label, /A11/i);
  ...
  try {
    await withServer(
      ...
      async (baseUrl) => {
        ...
        assert.equal(result.json.provider, 'cartesia');
        assert.equal(result.json.via, 'cartesia-tts');
        assert.match(result.json.voiceReference.label, /Vivy/i);
        assert.match(result.json.audio_url, /^\/api\/tts\/out\/tts-out-\d+-cartesia\.mp3$/);
        assert.equal(cartesiaBodies.length, 1);
...
test('tts async official auto voices keep Cartesia explicit-only and use local official reference when ElevenLabs is absent', async () => {
  ...
  global.fetch = async (url, options = {}) => {
    const value = String(url);
    if (value === 'https://api.cartesia.test/tts/bytes') {
      throw new Error('cartesia_should_not_run_for_official_auto_voice');
    }
    if (value === 'http://voice-bridge.test/api/voice/synthesize') {
      return {
        ok: false,
    ...
    if (value === 'http://voice-bridge.test/api/voice/convert') {
      ...
      return {
        ...
        async arrayBuffer() {
          return createPcm16Wav({ frequency: 180 });
        },
      };
    }
    if (value === 'http://a11-voice:5002/api/tts') {
      throw new Error('piper_http_should_not_run_for_official_auto_voice');
    }
    return previousFetch(url, options);
  };
...
test('tts out local generated assets survive repeated browser reads', async () => {
  ...
  try {
    await withServer(
      ...
      async (baseUrl) => {
        const first = await previousFetch(`${baseUrl}/api/tts/out/${filename}`);
        ...
        const second = await previousFetch(`${baseUrl}/api/tts/out/${filename}`);
...
test('tts official sample route is private by default', async () => {
  ...
  const previousEnv = {
    A11_RUNTIME_ROOT: process.env.A11_RUNTIME_ROOT,
    A11_VOICE_REFERENCE_DIR: process.env.A11_VOICE_REFERENCE_DIR,
    A11_VOICE_REFERENCE_LIBRARY_DIRS: process.env.A11_VOICE_REFERENCE_LIBRARY_DIRS,
    A11_VOICE_REFERENCE_LIBRARY_DIR: process.env.A11_VOICE_REFERENCE_LIBRARY_DIR,
    A11_VOICE_REFERENCE_LIBRARY_PATHS: process.env.A11_VOICE_REFERENCE_LIBRARY_PATHS,
    A11_VOICE_REFERENCE_LIBRARY_DISABLED: process.env.A11_VOICE_REFERENCE_LIBRARY_DISABLED,
