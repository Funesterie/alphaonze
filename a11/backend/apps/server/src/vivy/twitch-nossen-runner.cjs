...
function buildTwitchLyricsRequest({ winner, routing, seed }) {
  ...
  return [
    ...
    'Pour un duo ou plusieurs voix, donne un rôle lisible à chaque voix dans les balises quand c’est utile, par exemple [Verse 1 - voix masculine] puis [Verse 2 - voix féminine], avec alternance question-réponse au pré-refrain et refrain commun si le sujet le demande.',
