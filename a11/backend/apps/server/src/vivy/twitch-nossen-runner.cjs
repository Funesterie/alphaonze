...
function sanitizeTwitchLyricsForPromptLeakage(lyrics = '') {
  ...
  const forbidden = [
    ...
    /\blecture\s+s[ée]mantique\b/i,
    /\bconserver\s+les\s+indices\s+du\s+sujet\b/i,
    /\bne\s+pas\s+inventer\s+un\s+h[ée]ros\s+intime\b/i,
    /\ble\s+mot\s+[«"'’]?\s*histoire\s*[»"'’]?\s+(?:est|reste)\s+ambigu/i,
    /\b(?:je\s+)?serre\s+le\s+motif\s+central\b/i,
    /\ble\s+motif\s+central\b/i,
    /\bd[ée]cor\s+de\s+la\s+sc[èe]ne\b/i,
