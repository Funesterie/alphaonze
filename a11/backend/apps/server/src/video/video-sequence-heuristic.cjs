...
function resolveDefaultIdentityLocks(motionProfile = 'generic', prompt = '') {
  ...
  switch (String(motionProfile || 'generic').trim() || 'generic') {
    case 'power_up_loop':
    case 'transformation_rise':
      return ['same face', 'same outfit', 'same core identity', 'same silhouette'];
    case 'mounted_archery':
      return ['same rider', 'same horse', 'same saddle'];
    case 'archery_shot':
      return ['same face', 'same outfit', 'same core identity', 'same shooting stance'];
    case 'walk_cycle':
    case 'run_cycle':
      ...
      return ['same face', 'same outfit', 'same core identity', 'coherent environment'];
    default:
      return subjectType === 'humanoid'
        ? ['same face', 'same outfit', 'same core identity', 'same silhouette', 'coherent environment']
        : ['same core identity', 'same silhouette', 'coherent environment'];
  }
}
