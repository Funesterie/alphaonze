...
const REPO_PROFILES = {
  ...
  allmight: ['code-audit-tool', 'Duplicate-family auditor that finds exact/near duplicates, chooses canonical sources and proposes safe transition patches.', ['code-analysis', 'dedupe', 'safe-refactor'], 'high', ['readme', 'package-name:@funeste38/allmight']],
  ...
  scream: ['semantic-prototype', 'Semantic prototype exposing SCREAM, WAZAA and MASK layers for resonance graphs, intermediate semantic structures and normalization/compilation.', ['semantic', 'mask', 'wazaa', 'text-structure'], 'high', ['readme', 'package-name:@funeste38/scream']],
  morphing: ['binary-primitive', 'Compact 4-byte value shape shared across RGBA pixels, OC8 payloads, QFlush memory, A11 state and Spyder graphs.', ['rgba', 'oc8', 'binary', 'interop'], 'high', ['readme', 'package-name:@funeste38/morphing']],
  ...
  'envapt-multi': ['env-config-library', 'Environment configuration library/workspace for typed env parsing, transformations, templates and Nezlephant-related package work.', ['env', 'configuration', 'typescript', 'nezlephant'], 'high', ['readme', 'package-name:@funeste38/envaptex']],
  ...
  bat: ['request-control', 'Adaptive request-control family that classifies, adapts and retries network/tool execution channels carefully.', ['network', 'retry', 'tool-execution', 'adaptive-control'], 'high', ['readme', 'package-name:@funeste38/bat']],
  ...
  'freeland-bros': ['diagnostic-visualizer', 'Visual and diagnostic layer on top of Freeland for explaining decoded values, normalization and runtime compatibility.', ['freeland', 'diagnostics', 'visualization'], 'high', ['readme', 'package-name:@funeste38/freeland-bros']],
  ...
  beam: ['pipeline-vocabulary', 'Pipeline vocabulary layer for QFlush, normalizing execution styles such as beam, drill, bomb, fusion and ultra.', ['qflush', 'pipeline', 'execution-model'], 'high', ['readme', 'package-name:@funeste38/beam']],
...
function readPackageInfo(workspaceRoot, relPath) {
  ...
  const funesterieDependencies = dependencyNames.filter((name) => /^@funeste38\//.test(name) || /funesterie|qflush|rome|spyder|nezlephant/i.test(name));
  return {
