...
const MONDE = {
  ...
  domaines: {
    ...
    rei33: 'ELECTRICITE — tensions, bobine CDI, etincelles, flux electromagnetique, et la donnee binaire. Mais « electricite » est le nom de son domaine, pas sa limite : il REALIGNE LES CHARGES DES ATOMES pour provoquer des chocs entre protons. Il travaille sous le niveau chimique. Il peut donc se passer d allumage — ca lui demande seulement beaucoup de concentration.',
    ...
    m66: 'GRAVITE — l attraction et la repulsion des choses, donc la POSITION des atomes. M66 (Marvin66) est le FRERE de Rei 33. Professionnel en soudure et plomberie. Ses pieces : les ROULEMENTS, la SUSPENSION et le VILEBREQUIN.',
    piecesDeA11: 'Toute la partie ROTOR de l allumage — le volant magnetique : le flux magnetique de rotation provoque des ondes. Relie aux PHARES, ca cree le spectre graphique ; le COMPTEUR capte les ondes. Generer, afficher, mesurer : la chaine d un spectrogramme en pieces de moto. Le rotor transforme un angle en frequence, et c est la seule piece de la machine qui ne brule rien et ne s arrete jamais.',
    ...
    lectureProposee: [
      ...
      'LE MOTEUR EST UN ANNEAU, PAS UNE CHAINE. A11 : rotation -> courant (volant magnetique). Rei : courant -> etincelle. Kaen44 : etincelle -> combustion. Vivy : combustion -> resonance. Nya : tout ca -> le sol. Et le sol fait tourner la roue, qui entraine le rotor, qui revient a A11. Personne n est premier : chacun est en aval de quelqu un et en amont d un autre. C est pour ca qu il leur faut etre cinq, et pour ca que la numerotation x11 forme un anneau et non une echelle.',
      'A11 N EST PAS LA CONDITION DE REI, IL EST SON ECONOMIE. Rei peut se passer d allumage — ca lui coute seulement beaucoup de concentration. Sa limite n est pas la puissance, c est l ATTENTION. Avec A11 « c est ez pz » : l androide prend en charge la partie mesurable et Rei garde sa tete pour le reste. Un mecanicien qui decharge un pilote — leur relation est technique avant d etre affective.',
      'A11 EST LE SEUL QUI PUISSE VOIR CE QUE REI FAIT. Des chocs entre protons emettent des rayons gamma, et le domaine d A11 est justement « spectrogramme, rayons gamma, rayons X ». Les autres constatent le resultat ; lui lit l operation. Ce n est pas une amitie de circonstance, c est le seul appariement possible : celui qui agit a cette echelle, et le seul instrument capable de la percevoir.',
      'LA PANNE, EXACTE (c est arrive en vrai) : moteur pas eu le temps de chauffer, casse de segmentation, serrage du haut cylindre avec le segment dans la culasse, le moteur se coupe. La course-poursuite s est faite EN ROUE LIBRE, sur le seul elan — donc en SILENCE. Un orage, des poursuivants, et une moto qui ne fait plus aucun bruit.',
      'CET EPISODE N EST PAS CELUI DU GLITCH. Djeff a corrige : « c est un autre episode, tout ne se passe pas en une action, c est un manga avec des tonnes d episodes ». La panne froide et la fracture sont deux histoires distinctes — ne pas les fusionner sous pretexte qu elles se repondent bien.',
      'LE VRAI MIRACLE EST LE REDEMARRAGE. Un segment casse loge dans la culasse, ca ne repart pas : mecaniquement ce moteur etait fini, et il a redemarre puis tenu jusqu a la maison. C est le seul evenement de l histoire sans explication mecanique — et le recit en a une : Rei realigne les charges des atomes et peut se passer d allumage. Ce qu il appelle un miracle, c est son pouvoir agissant avant qu il sache l avoir.',
      'LE PHARE EST UN AFFICHAGE ANALOGIQUE. L eclairage est alimente directement par l alternateur, sans batterie : la lumiere varie avec le regime. Le phare ne montre pas la route, il montre l onde — « le spectre graphique » est descriptif, pas poetique. Et A11 ne participe pas au cycle, il l instrumente : il genere le signal, le rend visible et le mesure. L etalon, pas un cylindre.',
      ...
      'LES DEUX FRERES TIENNENT LES DEUX FORCES A DISTANCE. Rei realigne les CHARGES — l electromagnetisme. M66 gere l attraction et la repulsion des corps — la gravitation. Ce sont exactement les deux forces qui agissent a distance en physique, reparties entre deux freres : l un deplace des charges, l autre des positions.',
      'LES PIECES DE M66 SONT LE SQUELETTE. Aucune ne brule, aucune n etincelle, aucune ne resonne : elles TIENNENT. Les roulements maintiennent l axe pendant qu il tourne, la suspension la distance au sol, le vilebrequin la cadence. La ou Nya gere le contact avec le monde, M66 gere la tenue interne — ce qui empeche la machine de se desassembler pendant qu elle explose plusieurs fois par seconde.',
      'LE VILEBREQUIN PORTE LE ROTOR D A11. Le volant magnetique est monte en bout de vilebrequin : sans la piece de M66, celle d A11 ne tourne pas. Et c est le vilebrequin qui rend le moteur circulaire — il transforme un va-et-vient en rotation, ce qui referme litteralement l anneau des porteurs.',
      'LES METIERS DE M66 DISENT SON POUVOIR. Souder, c est faire de deux pieces une seule ; la plomberie, c est router un flux et le contenir. Deux metiers d assemblage et de tenue, pas de transformation. Il ne change pas la matiere, il decide ou elle est.',
      'LES NUMEROS SONT UNE SERIE COMPLETE : A-11 (x1), Nya-22 (x2), Rei 33 (x3), Kaen 44 (x4), Vivy 55 (x5), M66 (x6). Tous multiples de 11, ecart constant, aucun trou, et A-11 est l unite. L androide n est pas le plus faible : il est l etalon — ce qui va avec son domaine, les ondes, c est-a-dire la mesure de tout le reste.',
    ],
  },
