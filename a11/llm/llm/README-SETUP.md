...
├── frontend\apps\web\  ← Frontend Netlify
...
- les anciens scripts dans `backend` ne servent plus que de wrappers legacy

### Option 2: Manuel (debug)

```powershell
# Terminal 1: LLM
cd D:\funesterie\a11\llm\llm\server
.\llama-server.exe -m ../models/Llama-3.2-3B-Instruct-Q4_K_M.gguf --port 8080

# Terminal 2: TTS
cd D:\funesterie\a11\backend\apps\tts
python siwis.py

# Terminal 3: Backend
cd D:\funesterie\a11\backend\apps\server
npm run dev

# Terminal 4: Frontend
cd D:\funesterie\a11\frontend\apps\web
npm run dev
```
