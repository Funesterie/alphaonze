...
import { buildApiUrlFromBase, getCurrentApiBase, getAuthToken } from "./api";
...
function getApiUrl(path: string) {
  return buildApiUrlFromBase(getCurrentApiBase() || '/api', path);
}
...
async function jsonFetch(url: string, init?: RequestInit) {
  const authHeaders = getVsAuthHeaders();
  if (!authHeaders) {
  ...
  const htmlResponse = !isJson && /<!doctype html|<html/i.test(String(payload || ''));
  if (htmlResponse) {
    throw new Error(`A11Host indisponible pour cette vue (${res.status}). L'interface a recu une page HTML au lieu de l'API.`);
  }
...
export async function writeFile(path: string, content: string): Promise<FileWriteResponse> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/fs/write'), { method: 'POST', body: JSON.stringify({ path, content }) });
  return data;
}
...
export async function testDir(dir: string): Promise<{ ok: boolean; report?: string }> {
  ...
  const data = await jsonFetch(getApiUrl(`/api/v1/fs/test?dir=${encodeURIComponent(dir)}`));
  return data;
}
...
export async function fileExists(path: string): Promise<{ ok: boolean; exists: boolean; path?: string }> {
  ...
  const data = await jsonFetch(getApiUrl(`/api/v1/fs/exists?path=${encodeURIComponent(path)}`));
  return data;
}
...
export async function listFiles(dir: string): Promise<{ ok: boolean; items: Array<{ name: string; dir: boolean; file: boolean }> }> {
  ...
  const data = await jsonFetch(getApiUrl(`/api/v1/fs/list?dir=${encodeURIComponent(dir)}`));
  return data;
}
...
export async function getCompilationErrors(): Promise<CompilationError[]> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/compilation-errors'));
  return data.errors || [];
}
...
export async function getProjectStructure(): Promise<ProjectStructure> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/project-structure'));
  return data;
}
...
export async function getSolutionInfo(): Promise<SolutionInfo> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/solution-info'));
  return data;
}
...
export async function getActiveDocument(): Promise<ActiveDocument> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/active-document'));
  return data;
}
...
export async function getCurrentSelection(): Promise<string> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/current-selection'));
  return data.text || '';
}
...
export async function insertAtCursor(text: string): Promise<boolean> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/insert-at-cursor'), { method: 'POST', body: JSON.stringify({ text }) });
...
export async function replaceSelection(newText: string): Promise<boolean> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/replace-selection'), { method: 'POST', body: JSON.stringify({ newText }) });
  return data.success || false;
}
...
export async function deleteFile(path: string): Promise<boolean> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/file'), { method: 'DELETE', body: JSON.stringify({ path }) });
  return data.success || false;
}
...
export async function renameFile(oldPath: string, newPath: string): Promise<boolean> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/file/rename'), { method: 'PUT', body: JSON.stringify({ oldPath, newPath }) });
  return data.success || false;
}
...
export async function getWorkspaceRoot(): Promise<string> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/workspace-root'));
  return data.root || '';
}
...
export async function openFile(path: string): Promise<boolean> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/open-file'), { method: 'POST', body: JSON.stringify({ path }) });
  return data.success || false;
}
...
export async function gotoLine(path: string, line: number): Promise<boolean> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/goto-line'), { method: 'POST', body: JSON.stringify({ path, line }) });
  return data.success || false;
}
...
export async function buildSolution(): Promise<boolean> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/build'), { method: 'POST' });
  return data.success || false;
}
...
export async function getOpenDocuments(): Promise<string[]> {
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/open-documents'));
  return data.documents || [];
}

export async function executeShell(command: string): Promise<ExecuteShellResponse> {
  if (hostVs?.ExecuteShell) {
    const output = await hostVs.ExecuteShell(command);
    return {
      ok: true,
      command,
  ...
  const data = await jsonFetch(getApiUrl('/api/v1/vs/execute-shell'), { method: 'POST', body: JSON.stringify({ command }) });
  return {
    ok: data.ok !== false,
    command: String(data.command || command || '').trim(),
    output: String(data.output || '').trim(),
    stdout: String(data.stdout || '').trim(),
    stderr: String(data.stderr || '').trim(),
    exitCode: Number.isFinite(data.exitCode) ? Number(data.exitCode) : 0,
