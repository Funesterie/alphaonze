...
function compileMaskToPython(mask) {
  ...
  if (mask.task.domain === 'python' && mask.task.action === 'generate') {
    const prompt = String(mask?.inputs?.prompt || mask?.raw || '').trim();
    if (!prompt) {
      throw new Error('MASK.inputs.prompt required');
    }
    return [
      '"""',
      'Generated from A11 unified intent pipeline.',
      `User request: ${prompt.replace(/"""/g, '\\"\\"\\"')}`,
      '"""',
      '',
      'def main():',
      '    # TODO: implement the requested Python behavior.',
      '    pass',
      '',
