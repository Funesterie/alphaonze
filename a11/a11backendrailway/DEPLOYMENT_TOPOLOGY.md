...
5. Local Python runtime -> `a11llm/scripts/venv`
...
- Keep `a11llm` for heavy local assets only: GGUF files, `llama.cpp`, local Python venvs, and other machine-local runtimes.
- Keep lightweight backend-owned scripts inside `a11backendrailway/apps/server`.
