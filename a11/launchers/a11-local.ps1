...
function Build-ServiceDefinitions {
  ...
  $translationBaseUrl = (Get-ConfigValue $Config 'A11_TRANSLATION_BASE_URL' '').Trim()
  $translationModel = (Get-ConfigValue $Config 'A11_TRANSLATION_MODEL' '').Trim()
  $translationAllowAnon = (Get-ConfigValue $Config 'A11_TRANSLATION_ALLOW_ANON' '').Trim()
  ...
  $imageDefaultHeight = (Get-ConfigValue $Config 'A11_IMAGE_DEFAULT_HEIGHT' '').Trim()
  $visionProvider = (Get-ConfigValue $Config 'A11_VISION_PROVIDER' '').Trim()
  $janusEnabled = (Get-ConfigValue $Config 'A11_JANUS_ENABLED' '').Trim()
  $janusModelId = (Get-ConfigValue $Config 'A11_JANUS_MODEL_ID' '').Trim()
  $janusModelDir = (Get-ConfigValue $Config 'A11_JANUS_MODEL_DIR' '').Trim()
  $janusPythonPath = (Get-ConfigValue $Config 'A11_JANUS_PYTHON_PATH' '').Trim()
  $janusScriptPath = (Get-ConfigValue $Config 'A11_JANUS_SCRIPT_PATH' '').Trim()
  $janusDevice = (Get-ConfigValue $Config 'A11_JANUS_DEVICE' 'cuda').Trim()
  $janusTorchDtype = (Get-ConfigValue $Config 'A11_JANUS_TORCH_DTYPE' 'auto').Trim()
  $janusPreferLatest = (Get-ConfigValue $Config 'A11_JANUS_PREFER_LATEST' '').Trim()
  $imageReferenceJanusMaxTokens = (Get-ConfigValue $Config 'A11_IMAGE_REFERENCE_JANUS_MAX_TOKENS' '700').Trim()
  $imageReferenceJanusTimeoutMs = (Get-ConfigValue $Config 'A11_IMAGE_REFERENCE_JANUS_TIMEOUT_MS' '12000').Trim()
  ...
  $videoMaxRenderSide = (Get-ConfigValue $Config 'A11_VIDEO_MAX_RENDER_SIDE' '2048').Trim()
  $videoDefaultWidth = (Get-ConfigValue $Config 'A11_VIDEO_DEFAULT_WIDTH' '').Trim()
  $videoDefaultHeight = (Get-ConfigValue $Config 'A11_VIDEO_DEFAULT_HEIGHT' '').Trim()
  ...
  $videoUseJanusFrameAnalysis = (Get-ConfigValue $Config 'A11_VIDEO_USE_JANUS_FRAME_ANALYSIS' 'false').Trim()
